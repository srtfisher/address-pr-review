...
Then, on a branch with an open PR, ask your agent to use `/code-review-feedback`, or just ask in plain words:

```text
> Work through the review feedback on this PR.
> Address the comments on #482.
> /code-review-feedback https://github.com/acme/storefront/pull/482
```
...
The skill drives a small CLI at `skills/code-review-feedback/app/cli.mjs`. It's also published to npm, so you can run it without the skill, e.g. `npx code-review-feedback feedback --pr 482`:
...
`npm run build` writes the UI and the bundled CLI into `skills/code-review-feedback/app/`, which is committed so installs work without a build. CI fails if it's out of date. The fixture is generated by `node test/fixtures/generate.ts`, and the screenshots above are taken from it.

To release, bump `version` in `package.json`, commit, and push a matching tag (`git tag v0.2.0 && git push origin v0.2.0`). The release workflow runs the checks and publishes to npm through trusted publishing.
